...
exports.processAddonsSelectionNode = void 0;
const Log_1 = require("../../../utils/logger/Log");
const logger = (0, Log_1.createLogger)(module);
function parseAddonSelections(userInput, addonsMenu) {
    const selected = [];
    ...
    if (quantityMatches.length > 0) {
        quantityMatches.forEach(match => {
            ...
            const matchingAddons = addonsMenu.filter(addon => {
                ...
                if (/window/i.test(keyword))
                    return /window/i.test(type) || /window/i.test(label);
                if (/walk.?in/i.test(keyword))
                    return /walkin|walk.?in/i.test(type);
                if (/door/i.test(keyword) && !/walk/i.test(keyword)) {
                    return (/door/i.test(type) && !/walk/i.test(type));
                }
                if (/brace|anchor/i.test(keyword))
                    return /brace|anchor/i.test(type);
                if (/cupola/i.test(keyword))
                    return /cupola/i.test(type);
                if (/truss/i.test(keyword))
                    return /truss/i.test(type);
                return false;
            });
...
const processAddonsSelectionNode = async (state) => {
    ...
    try {
        ...
        let addonsMenu = state.addonsMenu || [];
...
exports.processAddonsSelectionNode = processAddonsSelectionNode;
//# sourceMappingURL=ProcessAddonsNode.js.map