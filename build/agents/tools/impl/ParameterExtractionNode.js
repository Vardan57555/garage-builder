...
exports.extractParametersNode = void 0;
...
async function extractParametersWithUnifiedPrompt(context, currentField, currentParams) {
    try {
        ...
        const calculation = DimensionCalculator_1.DynamicGarageDimensionCalculator.calculateDimensionsFromInput(context);
...
🔒 LOCKED FIELDS (DO NOT INCLUDE IN OUTPUT):`;
            ...
            if (currentParams.garage_type)
                lockedFields.push(`garage_type`);
            if (currentParams.width)
                lockedFields.push(`width`);
            if (currentParams.length)
                lockedFields.push(`length`);
            if (currentParams.height)
                lockedFields.push(`height`);
            if (currentParams.state_name)
                lockedFields.push(`state_name`);
            if (currentParams.roof_type)
                lockedFields.push(`roof_type`);
            if (currentParams.gauge)
                lockedFields.push(`gauge`);
            if (currentParams.building_type)
                lockedFields.push(`building_type`);
...
User input: "${context}"`;
        ...
        if (response.includes("def ") ||
            ...
            logger.warn(`[extractParametersWithUnifiedPrompt] LLM returned code instead of JSON`);
            return "{}";
        }
        return response;
    }
...
const extractParametersNode = async (state) => {
    ...
    try {
        ...
        const rawParams = await extractParametersWithUnifiedPrompt(context, state.currentField, currentParams);
        ...
        if (garageTypeChanged) {
            ...
            const calc = DimensionCalculator_1.DynamicGarageDimensionCalculator.calculateDimensionsFromInput(extractedParams.garage_type);
        ...
        if (mergedParams.garage_type && !mergedParams.width) {
            ...
            const calc = DimensionCalculator_1.DynamicGarageDimensionCalculator.calculateDimensionsFromInput(mergedParams.garage_type);
        ...
        if (mergedParams.state_name) {
            ...
            const validationResult = await StateValidator_1.StateDataValidator.validateState(mergedParams.state_name, async (name) => await LeadAgentHelpers_1.LeadAgentHelpers.mapStateToDB(name, state.stateMapCache));
        ...
        if (mergedParams.roof_type) {
            ...
            const validationResult = await RoofValidator_1.RoofDataValidator.validateRoofType(mergedParams.roof_type);
            if (!validationResult.isValid) {
        ...
        if (carCountMatch) {
            ...
            const calc = DimensionCalculator_1.DynamicGarageDimensionCalculator.calculateDimensionsFromInput(`${numCars}-car`);
            if (calc.width && calc.length && calc.height) {
...
exports.extractParametersNode = extractParametersNode;
//# sourceMappingURL=ParameterExtractionNode.js.map