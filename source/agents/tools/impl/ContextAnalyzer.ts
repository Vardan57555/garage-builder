...
export class ContextAnalyzer {
     ...
     */
    public async analyzeNumericInput(userInput: string, currentField: string | null, conversationHistory: string): Promise<"state" | "dimension" | "unclear">
    {
        try {
            const trimmedInput: string = userInput.trim();
            const isNumeric: boolean = Constants.NUMERIC_PATTERN.test(trimmedInput);

            if (currentField === "state_name" && isNumeric && trimmedInput.length <= Constants.MAX_STATE_LENGTH)
            {
                logger.info(`[ContextAnalyzer] Numeric input "${userInput}" in state context - likely STATE`);
                return "state";
            }

            if (currentField && !Constants.DIMENSION_FIELDS.has(currentField) && isNumeric)
            {
     ...
     */
    private async analyzeWithAI(userInput: string, currentField: string | null, conversationHistory: string): Promise<"state" | "dimension" | "unclear">
    {
        try {
            const prompt: string = this.buildAnalysisPrompt(userInput, currentField, conversationHistory);
     ...
     */
    private buildAnalysisPrompt(userInput: string, currentField: string | null, conversationHistory: string): string
    {
        return `You are analyzing user intent based on conversation context.
...
RESPOND WITH ONLY ONE WORD: current_field | dimension | unclear`;
     ...
     */
    private parseAIResponse(response: any): "state" | "dimension" | "unclear"
    {
        const result = response.toLowerCase().trim();
        logger.info(`[ContextAnalyzer] AI analysis result: "${result}"`);

        if (result.includes(Constants.AI_RESPONSE_KEYWORDS.dimension))
        {
            return "dimension";
        }

        if (result.includes(Constants.AI_RESPONSE_KEYWORDS.current))
        {
            return "state";
        }
...
export class ExtractionResultBuilder {
     ...
     */
    static error(userFriendlyParams: Partial<UserFriendlyParams>, currentField: string | null, errorMessage: string, responseMessage: string): ExtractionResult
    {
        return {
            userFriendlyParams,
            currentField,
     ...
     */
    static success(userFriendlyParams: Partial<UserFriendlyParams>, currentField: string | null, responseMessage: string, nextStep: string = "check_missing_fields"): ExtractionResult
    {
        return {
            userFriendlyParams,
            currentField,
