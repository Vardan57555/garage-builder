...
/**
 * Generate SVG visualization of garage (all on ONE line)
 */
...
/**
 * Main format function - SVG comes FIRST with clear markers
 */
function formatFinalQuoteWithVisualization(
    params: any,
    basePrice: number,
    selectedAddons: any[],
    finalTotal: number,
...
/**
 * NODE: Generate visualization and show final quote
 */
export const generateGarageVisualizationNode = async (state: LeadAgentStateType) => {
    ...
    try {
        ...
        const response = formatFinalQuoteWithVisualization(
            params,
            basePrice,
            selectedAddons,
            finalTotal,
